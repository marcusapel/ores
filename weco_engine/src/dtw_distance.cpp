...
CostValue dtw_distance(const DataValue * data1,unsigned size1,const DataValue * data2,unsigned size2,int norm){
    if (norm ==2)
        return dtw_distance_l2(data1,size1,data2,size2);
    if (norm ==1)
        return dtw_distance_l1(data1,size1,data2,size2);
    return 1e300;
...
CostValue dtw_distance_l1(const DataValue * data1,unsigned size1,const DataValue * data2,unsigned size2) {
    ...
    for (unsigned i2 =1;i2<size2;i2++){
        ...
        cur[0] = prev[0] + std::abs(data1[0]-v2);
        for(unsigned i1=1;i1<size1;++i1)
...
CostValue dtw_distance_l2(const DataValue * data1,unsigned size1,const DataValue * data2,unsigned size2) {
    ...
    for (unsigned i2 =1;i2<size2;i2++){
        cur.swap(prev);
        const double v2 = data2[i2];
        cur[0] = prev[0] + square(data1[0]-v2);
        for(unsigned i1=1;i1<size1;++i1)
